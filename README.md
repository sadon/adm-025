...
8-13:
  - [ ] Тут нельзя охватить весь синтаксис, поэтому сослаться, что разные части синтаксиса будут рассмотрены в последующих блоках.
...
12: код - сниппет кода, так везде, может Алёна (в подчинении Ани) поможет?
...
14: убрать ссылку, они идет далее в 15 слайде
...
20:
  - [ ] аналогично к слайду 18: может поставить раньше
  первый - центральный aws account, второй региональны аккаунт

22:
  - [ ] Base commands -> Main commands
...
25*:
  - [ ] Добавить определение ресурса
  data source vs resource / https://discuss.hashicorp.com/t/general-question-about-data-sources-vs-resources/8071

27, 28:
  - [ ] изменить префикс упражнения
  28 - начать с 5. а не с 1.

30:
  - [ ] Где можно хранить https://www.terraform.io/docs/language/settings/backends/remote.html
  ...
  - `terraform force-unlock` надо вынести на слайд
  - что если одновременно выполнить terraform apply из разных терминалов?
  - сказать подробно про lock-механизм

31:
  - [ ] Разделить упражнения на два:
  1. Как переезжать из локального стейта в удалённый
  2. Управление состоянием - но тут идёт модификация состояния - хорошо/плохо?

28:
  - [ ] добавить упражнение: переименование ресурса + `terraform state mv`
...
33*
  - [ ] где-то после resource и импорта должен быть `data` - использование готовых ресурсов
...
36:
  - [ ] heredoc не указан в синтаксисе, а это полезная фишка.
...
36*:
  - [ ] Не хватает validation на слайдах

38*
  - [ ] Добавить слайд: Также потеряны функции, которых много
...
44:
  - [ ] я бы убрал картинку справа - не несёт полезной нагрузки

46:
  - [ ] update link to https://www.terraform.io/docs/cli/import/index.html
...
48-49:
  - [ ] notes по нумерации не сопадают с пунктами, трудно связать - синхронизировать

51-52:
  - [ ] Суффиксы названий отличаются от ex7 part 1, part 2.
  Плохо понимаю суть упражнения Ex8.
...
55:
  - [ ] default_tags variable should be a map -> перенести в пункт 1

61:
  - [ ] название и ссылка -> две разные ссылки -> исправить
  - [ ] сделать упражнение для `-replace` : сломать машину ¯\_(ツ)_/¯ и пересоздать её

63:
  - [ ] > Сколько бы вы не пытались объявить ресурсы в разных файлах внутри одной директории terraform всегда будет их склеивать.
...
66:
  - [ ] > один из приемов работы с терраформ (не слушайте мой вредный совет) – удалить папку –dot. .terraform
  ...
  звучит что Terraform не production качества продукт. Что действительно происходит если кэш не валидный? Разве он его сам не может обновить? -> расписать почему
...
77:
  - [ ] > Возможно вам потребуется работать с workspaces
...
80:
  - [ ] указать выбор нужной утилиты версии terraform (приём с PATH)
  <!-- - [ ] Полезная утилита tfswitch https://tfswitch.warrensbox.com/ -->

83:
  - backup you state -> your
